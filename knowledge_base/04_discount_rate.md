...
## Key Considerations
1. **Lease Term Match:** The discount rate must match the lease tenure (e.g., a 5-year lease should use a 5-year borrowing rate).
2. **Security & Asset Class:** The rate should reflect the collateral/security profile of the asset being leased.
3. **Economic Environment:** The rate should be currency-specific and location-specific (e.g., Indian Rupee borrowing rates for leases in India).
4. **Changes in Rate:** The discount rate is re-assessed only upon specific trigger events like lease modifications, changes in the lease term, or changes in purchase options.
